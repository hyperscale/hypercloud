...
export class ServiceDetailScalabilityComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        Object.keys(this.service.Spec.Labels).forEach(label => {
            ...
            switch (label) {
                case 'com.hyperpaas.autoscale.enable':
                    this.autoscale.enable = value === 'true';
                    break;
                case 'com.hyperpaas.autoscale.min':
                    this.autoscale.min = parseInt(value, 10) || 1;
                    break;
                case 'com.hyperpaas.autoscale.max':
    ...
    onScaleSubmit() {
        ...
        this.service.Spec.Labels['com.hyperpaas.autoscale.enable'] = this.autoscale.enable ? 'true' : 'false';
        this.service.Spec.Labels['com.hyperpaas.autoscale.min'] = `${this.autoscale.min}`;
        this.service.Spec.Labels['com.hyperpaas.autoscale.max'] = `${this.autoscale.max}`;
